...
Patch validation runs in the defects4j_docker_web service (default
http://localhost:8090); start it before a run and override the URL with
$DEFECTS4J_URL or `run --d4j-url`.
"""
import json
import locale
import os
...
import d4j_client  # noqa: E402  # HTTP client for the defects4j_docker_web service
...
def checkout_bug(project: str, bug_index: str):
    """Check out a buggy project version via the defects4j_docker_web service.

    The checkout runs inside the Docker container; its output lands in the
    bind-mounted workspace, so the agent sees it on the host.
    """
    folder_name = d4j_client.folder_name_for(project, bug_index)
    write_to = SCRIPT_DIR / "auto_gpt_workspace" / folder_name
    # Clear any stale checkout on the host (== the container's view).
    if write_to.exists():
        shutil.rmtree(write_to)

    rc, stdout, stderr = d4j_client.checkout(project, bug_index, folder_name)
    if rc != 0:
        console.print(f"  [red]Checkout failed: {stderr}[/red]")
        raise RuntimeError(
            f"defects4j checkout failed for {project} {bug_index}: {stderr}"
        )
...
def execute_run(bugs: list[tuple[str, str]], model: str, hyperparams: str, max_cycles: int, temperature: float = 0.0):
    ...
    setup_defects4j_env()
    d4j_client.set_host_workspace(SCRIPT_DIR / "auto_gpt_workspace")

    # Pre-flight: verify the defects4j_docker_web validation service is reachable.
    if not d4j_client.health_check():
        console.print(
            f"  [red]Cannot reach the defects4j validation service at "
            f"{d4j_client.D4J_URL}[/red]"
        )
        console.print("  Start it with:  cd defects4j_docker_web && docker compose up -d")
        console.print("  Or set DEFECTS4J_URL / pass --d4j-url to point at the service.")
        sys.exit(1)
    console.print(f"  Validation service: [green]{d4j_client.D4J_URL}[/green]")
...
def interactive_run():
    ...
    console.print(f"  Validation service: [bold]{d4j_client.D4J_URL}[/bold]")
    console.print()
...
@click.option("--d4j-url", default=None,
              help="defects4j_docker_web service URL (default $DEFECTS4J_URL or http://localhost:8090)")
def run(bugs, bugs_file, model, temperature, hyperparams, max_cycles, d4j_url):
    """Run RepairAgent on one or more bugs."""
    if hyperparams is None:
        hyperparams = str(SCRIPT_DIR / "hyperparams.json")

    if d4j_url:
        d4j_client.set_url(d4j_url)
