...
def create_chat_completion(
    ...
    if is_anthropic_model(model):
        ...
        # vLLM-hosted models (e.g. Qwen via a custom api_base) return an empty
        # `content` when thinking mode is on — the text goes to a separate
        # `reasoning` field, which breaks the agent's JSON parsing. The legacy
        # openai lib forwards unknown kwargs straight into the request body, so
        # we pass vLLM's chat-template flag to disable thinking. Gated to
        # non-OpenAI endpoints so the official API is unaffected.
        _api_base = chat_completion_kwargs.get("api_base") or ""
        if _api_base and "api.openai.com" not in _api_base:
            chat_completion_kwargs["chat_template_kwargs"] = {"enable_thinking": False}
