...
def is_anthropic_model(model: str) -> bool:
    """Check if a model should be routed through the Anthropic provider.

    Includes native Anthropic Claude models and DeepSeek models served via
    DeepSeek's Anthropic-compatible surface at
    ``https://api.deepseek.com/anthropic`` (e.g. ``deepseek-v4-flash``).
    """
...
def create_chat_completion(
    ...
    **kwargs,
    ...
    # DeepSeek's Anthropic-compatible endpoint reuses the Anthropic SDK with a
    # different api_key / base_url.  Route deepseek-* through it; everything
    # else stays on api.anthropic.com.
    is_deepseek = model.startswith("deepseek-")
    if is_deepseek:
        api_key = kwargs.pop("api_key", None) or os.getenv("DEEPSEEK_API_KEY") \
                  or os.getenv("ANTHROPIC_API_KEY")
        base_url = os.getenv("DEEPSEEK_ANTHROPIC_BASE_URL",
                             "https://api.deepseek.com/anthropic")
        if not api_key:
            raise ValueError(
                "DEEPSEEK_API_KEY is not set. Required for deepseek-* models."
            )
    else:
        api_key = kwargs.pop("api_key", None) or os.getenv("ANTHROPIC_API_KEY")
        base_url = None  # SDK default (api.anthropic.com)
        if not api_key:
            raise ValueError(
